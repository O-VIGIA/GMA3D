...
def testing(args):
    ...
    # with occlution
    elif args.dataset == 'FT3DO':
        from datasets.flyingthings3d_flownet3d import FT3D
        folder = 'data_processed_maxcut_35_20k_2k_8192'
        dataset_path = os.path.join(args.root, 'data', folder)
        test_dataset = FT3D(root_dir=dataset_path, nb_points=args.max_points, mode='test')        
    elif args.dataset == 'KITTIO':
        from datasets.kitti_flownet3d import Kitti
        folder = 'kitti_rm_ground'
        dataset_path = os.path.join(args.root, 'data', folder)
        test_dataset = Kitti(root_dir=dataset_path, nb_points=args.max_points)
    ...
    for i, batch_data in enumerate(test_progress):
        ...
        pc_1_name = file_name + '/' + 'pc1.npy'
        pc_2_name = file_name + '/' + 'pc2.npy'
        flow_est_name = file_name + '/' + 'flow_est.npy'
        flow_gt_name = file_name + '/' + 'flow_gt.npy'
        
        [pcc1, pcc2]  = batch_data['sequence']
        pc_1 = pcc1.cpu().numpy()
        pc_2 = pcc2.cpu().numpy()
        flow_est = est_flow.cpu().numpy()
        flow_gt = batch_data["ground_truth"][1].cpu().numpy()
        
        np.save(pc_1_name, pc_1)
        np.save(pc_2_name, pc_2)
        np.save(flow_est_name, flow_est)
        np.save(flow_gt_name, flow_gt)
